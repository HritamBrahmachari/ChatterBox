...
const Messages = () => {
  ...
  useEffect(() => {
    ...
    <div className="messages-container overflow-y-auto" ref={messagesContainerRef}>
      {loading ? (
        [...Array(3)].map((_, idx) => (
        ...
        <div className="flex flex-col items-center justify-center h-full p-4">
          <div className="welcome-message glass-card-dark p-4 sm:p-6 rounded-lg text-center max-w-lg">
            <div className="flex justify-center mb-4">
              <div className="user-avatar-initials bg-gradient-to-r from-primary-600 to-primary-400 w-12 h-12 sm:w-16 sm:h-16 text-xl sm:text-2xl">
                CB
              </div>
            </div>
            ...
            <div className="space-y-2 text-left text-gray-300 mb-2 sm:mb-4">
        ...
        <div className="flex flex-col items-center justify-center h-full p-4">
          <div className="welcome-message glass-card-dark p-4 rounded-lg text-center">
            ...
            <p className="text-xs sm:text-sm text-gray-400">Say hello to start the conversation!</p>
          </div>
        </div>
      ) : (
        <>
          {/* System Welcome Message */}
          <div className="message-wrapper justify-center mb-4 sm:mb-6 px-2">
            <div className="system-message p-2 sm:p-3 rounded-lg bg-gray-800 bg-opacity-60 text-center max-w-[90%]">
              <div className="flex justify-center mb-1 sm:mb-2">
                <TiMessages className="text-lg sm:text-xl text-primary-400" />
              </div>
          ...
          {messages.map((message, idx) => {
            ...
            return (
              ...
              >
                {!isSender && (
                  <div className="user-avatar-initials flex-shrink-0 self-start mt-1 hidden xs:flex">
                    {getInitials(selectedConversation?.fullName)}
                  </div>
                )}
                ...
                {isSender && (
                  <div className="user-avatar-initials bg-primary-600 flex-shrink-0 self-start mt-1 hidden xs:flex">
...
const MessageSkeleton = () => (
  <div className="message-wrapper justify-start mb-4 animate-pulse">
    <div className="w-8 h-8 bg-gray-700 rounded-full hidden xs:block"></div>
    <div className="flex flex-col gap-1">
      <div className="h-10 w-48 sm:w-52 bg-gray-700 rounded-2xl rounded-tl-none"></div>
      <div className="w-16 sm:w-20 h-3 bg-gray-700 rounded-md ml-1"></div>
    </div>
  </div>
);
